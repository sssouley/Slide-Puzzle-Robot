...
def flush_and_execute():
    ...
    while latest_message is not None:  # Process all messages in the queue
        ...
        # After movement, return to facing east if not already
        if current_orientation != 'e':
            return_angle, _ = get_turn_angle_and_new_dir_math(1, 0, current_orientation)
            print(f"Returning to east. Turning {return_angle}° from {current_orientation} to 'e'")
            robot1.turn(return_angle, max_effort=0.5)
