...
if False: #publishing
    ...
    def handle_message(dx, dy):
        try:
            ...
            while abs(dxMovement) > 20:
                if dxMovement > 0:
                    robot3.set_effort(0.2,0.2) # walking until it's done
                if dxMovement < 0:
                    robot3.set_effort(-0.2, -0.2)
            robot3.stop()
            time.sleep(3)
    ...
    c.subscribe("SlidePuzzle/send/3") #can be re-named to any topic name and any data name e.g., nemitz/kinematics
    while True:
        try:
            c.wait_msg()
        except:
