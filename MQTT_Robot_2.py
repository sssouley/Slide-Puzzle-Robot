...
if False: #publishing
    ...
    def handle_message(dx, dy):
        try:
            ...
            while abs(dxMovement) > 20:
                if dxMovement > 0:
                    robot2.set_effort(0.2,0.2) # walking until it's done
                if dxMovement < 0:
                    robot2.set_effort(-0.2, -0.2)
            robot2.stop()
            time.sleep(3)
            while abs(dyMovement) > 20:
                if dyMovement > 0:
                    robot2.turn(90, max_effort=0.5)
                    robot2.set_effort(0.2,0.2)
                if dyMovement < 0:
                    robot2.turn(-90, max_effort= 0.5)
            robot2.stop()
    ...
    c.subscribe("SlidePuzzle/send/2") #can be re-named to any topic name and any data name e.g., nemitz/kinematics
    while True:
        try:
            c.wait_msg()
        except:
